...
public class JZTextureView extends TextureView {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (videoWidth == 0 || videoHeight == 0) {
            int vw = ((View) getParent()).getMeasuredWidth();
            int vh = ((View) getParent()).getMeasuredHeight();
            setMeasuredDimension(vw, vh);
            return;
        }

        if (VIDEO_IMAGE_DISPLAY_TYPE == Jzvd.VIDEO_IMAGE_DISPLAY_TYPE_RATIO) {
//            int vw = ((View) getParent()).getMeasuredWidth();
//            int vh = ((View) getParent()).getMeasuredHeight();

            int vw = LContext.screenWidth;
            int vh = LContext.screenHeight;
            float scale;
            if (videoWidth / videoHeight < vw / vh) {
                scale = vh * 1.0f / videoHeight;
            } else {
                scale = vw * 1.0f / videoWidth;
            }
            int targetW = (int) (videoWidth * scale);
            int targetH = (int) (videoHeight * scale);
        ...
        if (parentWidth != 0 && parentHeight != 0 && videoWidth != 0 && videoHeight != 0) {
            if (VIDEO_IMAGE_DISPLAY_TYPE == Jzvd.VIDEO_IMAGE_DISPLAY_TYPE_FILL_PARENT) {
        ...
        if (parentWidth != 0 && parentHeight != 0 && videoWidth != 0 && videoHeight != 0) {
            if (VIDEO_IMAGE_DISPLAY_TYPE == Jzvd.VIDEO_IMAGE_DISPLAY_TYPE_ORIGINAL) {
                /**原图**/
                height = videoHeight;
                width = videoWidth;
            } else if (VIDEO_IMAGE_DISPLAY_TYPE == Jzvd.VIDEO_IMAGE_DISPLAY_TYPE_FILL_SCROP) {
                if (viewRotation == 90 || viewRotation == 270) {
                    int tempSize = parentWidth;
                    parentWidth = parentHeight;
                    parentHeight = tempSize;
