...
//    textColor = typeArray.getColor(R.styleable.CircleProgressbar_textColor, 0xFFFFFF);
    ...
    private void init(Context ctx) {
        strokeWidth = DeviceUtil.dip2px(ctx, 5);
        ringColor = Color.parseColor("#FFA902");
        ...
        backgroundPaint = new Paint(ringPaint);
        backgroundPaint.setColor(Color.parseColor("#7fffffff"));
...
//    txtHeight = fm.descent + Math.abs(fm.ascent);
        ...
        oval = new RectF(0, 0, DeviceUtil.dip2px(ctx, 58), DeviceUtil.dip2px(ctx, 58));
    }
...
//            RectF oval = new RectF(0, 0, getMeasuredWidth(), getMeasuredHeight());
            canvas.drawRect(oval, ringPaint);
//            canvas.drawCircle(100, 100, 100, backgroundPaint);
