...
public class ClassficationPagerAdapter extends PagerAdapter {
    ...
    public ClassficationPagerAdapter(Context context, List<List<CategoryEntity>> list) {
        mContext = context;
        mData = list;
            ...
            for (int j = 0; j < spanCount; j++) {
                ...
                channelItemView.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        if (TextUtils.equals(channelEntity.getName(), LContext.getString(R.string.watch_more))) { //查看更多
                            EventBus.getDefault().post(new PushEvent(Constant.Event.CLASSFICATION_ITEM, channelEntity));
                        } else {
                            DataCache.firstCategory = DataCache.homeFirstCategory;
                            DataCache.thirdCategory = channelEntity;
                            GoodsListFrag.start(mContext, channelEntity._id, channelEntity.getName());
                        }
