...
import kotlinx.android.synthetic.main.group_order_item_fragment.*
import java.util.*
 ...
 * @author 银进
 */
class GroupOrderItemFragment : BaseFragment() {
    ...
    override fun getLayoutId() = R.layout.group_order_item_fragment
    ...
    override fun init(savedInstanceState: Bundle?) {
        ...
        orderViewModel?.dataResult?.observe(this, Observer {
            if (it?.tag == ApiHost.ORDER_LIST_GROUP && selfIndex == it.extra) {
                if (page == 1) {
                    //第一次加載
                    if (it.successful) {
    ...
    fun refreshData() {
        ...
        page = 1
        orderViewModel?.doOrderListGroup(orderViewModel?.orderItemSource?.value, selfIndex, row, page, orderViewModel?.orderMonth?.value, null, null)
    }
