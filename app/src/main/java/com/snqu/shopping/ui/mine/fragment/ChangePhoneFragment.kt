 ...
 * @author 银进
 */
class ChangePhoneFragment:SimpleFrag() {
    private val userViewModel by lazy {
        ViewModelProviders.of(this).get(UserViewModel::class.java)
    }
    override fun getLayoutId()= R.layout.change_phone_fragment
    ...
    override fun init(savedInstanceState: Bundle?) {
        ...
        userViewModel.dataResult.observe(this, Observer {
            if (it?.tag == ApiHost.CHANGE_PHONE_CODE) {
                if (it.successful) {
                    ChangePhoneMsgCodeFragment.start(activity,et_phone.text.toString())
                }
    ...
    private fun initView() {
        ...
        tv_send_msg_code.onClick {
            ...
            if (phone.matches(Regex(Constant.Match.PHONE_MATCH))) {
                userViewModel.doChangePhoneCode(phone)
            } else {
    ...
    companion object {
        fun start(context: Context?) {
            val fragParam = SimpleFragAct.SimpleFragParam("",
                    ChangePhoneFragment::class.java)
