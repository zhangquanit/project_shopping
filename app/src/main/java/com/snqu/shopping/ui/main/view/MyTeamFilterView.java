 ...
 * @author 张全
 */
public class MyTeamFilterView extends LinearLayout {
    private int d5;
    ...
    public MyTeamFilterView(Context context) {
        super(context);
        init(context);
    }

    public MyTeamFilterView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public MyTeamFilterView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
                ...
                public void onClick(View v) {
                    ...
                    switch (v.getId()) {
                        case R.id.tv_time: //直属注册时间
                            ...
                            tv_time.setText(getText("专属注册时间", selColor, drawRes));
                            tv_fans.setText(getText("粉丝人数", normolColor, R.drawable.filter_icon_normal));
                            tv_filter.setText(getText("累计预估收益", normolColor, R.drawable.filter_icon_normal));
                            break;
                        case R.id.tv_fans: //成员人数
                            drawRes = getSelDrawRes(pos);

                            tv_time.setText(getText("专属注册时间", normolColor, R.drawable.filter_icon_normal));
                            tv_fans.setText(getText("粉丝人数", selColor, drawRes));
                            tv_filter.setText(getText("累计预估收益", normolColor, R.drawable.filter_icon_normal));

                            break;
                        case R.id.tv_filter: //累计预估收益
                            drawRes = getSelDrawRes(pos);
                            tv_time.setText(getText("专属注册时间", normolColor, R.drawable.filter_icon_normal));
                            tv_fans.setText(getText("粉丝人数", normolColor, R.drawable.filter_icon_normal));
                            tv_filter.setText(getText("累计预估收益", selColor, drawRes));
                            break;
                    }
                    //选中文字加粗
//                    setTextBold(pos);
        ...
        tv_time.setText(getText("专属注册时间", selColor, R.drawable.filter_icon_down));
        tv_fans.setText(getText("粉丝人数", normolColor, R.drawable.filter_icon_normal));
        tv_filter.setText(getText("累计预估收益", normolColor, R.drawable.filter_icon_normal));
    }
    ...
    public FansQueryParam.QuerySort getSort() {
        switch (lastSelPos) {
            case 0: //直属注册时间
                sort = selIndex == 1 ? FansQueryParam.QuerySort.TIME_UP : FansQueryParam.QuerySort.TIME_DOWN;
                break;
            case 1: //成员人数
