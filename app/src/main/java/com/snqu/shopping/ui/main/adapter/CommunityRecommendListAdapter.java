 ...
 * @author 张全
 */
public class CommunityRecommendListAdapter extends BaseQuickAdapter<CommunityEntity, BaseViewHolder> {
    ...
    public CommunityRecommendListAdapter(CommunityListFrag communityListFrag) {
        super(R.layout.community_recommend_list_item);
    ...
    public void showOrderCount(boolean orderCount) {
        showOrderCount = orderCount;
    ...
    protected void convert(BaseViewHolder helper, CommunityEntity item) {
        ...
        item.setRankBg(iv_rank);
        //头像背景
        helper.setVisible(R.id.item_rank_bg, item.rank <= 3);

//        helper.setGone(R.id.item_rank_bar, item.rank != 0 && item.rank != 9999999);

        //商品排名
        TextView item_good_rank = helper.getView(R.id.item_good_rank);
        String rank = item.rank > 100 ? "100+" : String.valueOf(item.rank);
        SpannableStringBuilder stringBuilder = new SpanUtils()
                .append("商品排名:").setForegroundColor(Color.parseColor("#838387"))
                .append(rank).setForegroundColor(Color.parseColor("#F73737"))
                .create();
        item_good_rank.setText(stringBuilder);
        //下单排名
        helper.setGone(R.id.item_order_count, showOrderCount);
        TextView item_order_count = helper.getView(R.id.item_order_count);
        if (showOrderCount) {
            item_order_count.setText("下单量:" + item.order_count);
        } else {
            item_order_count.setText(null);
        }

        if (item.user_info != null) {
            GlideUtil.loadRoundPic(helper.getView(R.id.item_img), item.user_info.avatar, R.drawable.icon_default_head, R.drawable.icon_default_head, 16);
            helper.setText(R.id.item_recom, item.user_info.username);
        }
...
//        helper.setText(R.id.item_count, NumberUtil.INSTANCE.sellCount(item.counts));
        ...
        if (!item.hasImgs() && !hasVideo && null == item.getGoods()) { //无图片/无视频/无商品
            ...
            CommunityRecommendListAdapter.PicListAdapter picListAdapter = new CommunityRecommendListAdapter.PicListAdapter(hasVideo, resId);
